...
void SPHBase::compute_non_pressure_acceleration()
{
    ...
    for (auto& p : ps_.particles()) {
        ...
        for (auto& q : p->neighbors()) {
        // Prompt: use the "compute_viscosity_acceleration" function to compute 
        // the viscosity acceleration between p and q"
    ...
    const std::shared_ptr<Particle>& q)
{
    ...
    Vector3d laplace_v = 10 * (ps_.mass() / q->density()) * 
...
void SPHBase::compute_pressure_gradient_acceleration()
{
    #pragma omp parallel for
    for (auto& p : ps_.particles()) {
        // (HW TODO) Traverse all particles and compute each particle's 
        // acceleration from pressure gradient force
        
        for (auto& q : p->neighbors()) {
			// ... compute the pressure gradient force contribution from q to p
			double factor = -ps_.mass() * (p->pressure() / 
                (p->density() * p->density()) + q->pressure() / (q->density() * q->density()));
            p->acceleration_ += factor * grad_W(p->x() - q->x(), ps_.h());
        }
    }
}
